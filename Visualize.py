...
class Visualize(object):
    ...
    def update_plot(self, gridObj, f, axarr):
        ...
        cPopMap = ['#FFFFFF', '#E2E2E2', '#C6C6C6', '#AAAAAA', '#8D8D8D', '#717171', '#555555', '#383838', '#1C1C1C', cDead]
        convert = matplotlib.colors.ColorConverter()
            ...
            for i in range(0, width):
                for j in range(0, height):
                    ...
                    if cell.INITIAL_POP == 0: 
                        ...
                        if popColor > 9 or popColor < 0: popColor = 9 # Error check for out of bounds
                        if popColor == 9 and cell.INITIAL_POP != cell.TOTAL_DEAD: popColor = 8 # If theres humans left, dont be black
                        temp = N.array( convert.to_rgb( cPopMap[popColor] ) ) # grab the hex code belonging to that pop ratio
